...
# search elephant paths after finding a person path
# use dynamic programming/caching to speed up process
...
def search_for_route(time, location, visited, visits, on_elephant):
    ...
    for valve_name in important:
        ...
        new_time = time + simple_valve_graph[(location, valve_name)] + 1
        new_pressure =  (max_time - new_time) * valves[valve_name]["Flow Rate"]
        if(new_time < max_time):
            max_pressure = max(max_pressure, 
                search_for_route(new_time,
                    valve_name,
                    tuple(new_visited), 
                    visits + 1,
                    on_elephant) + new_pressure)
    if(not on_elephant):
        max_pressure = max(max_pressure, 
            search_for_route(0,
                "AA",
...
def main():
    ...
    print(search_for_route(0, "AA", visited, 0, False))
